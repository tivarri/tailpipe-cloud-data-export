...
# --- Cross-account CloudWatch access setup ---
echo "Fetching list of child accounts from AWS Organizations..."

child_accounts=$(aws organizations list-accounts --query "Accounts[?Status=='ACTIVE'].Id" --output text)

for account_id in $child_accounts; do
  echo "Assuming role in child account: $account_id"

  creds=$(aws sts assume-role \
    --role-arn arn:aws:iam::$account_id:role/CloudWatchReadRole \
    --role-session-name "TailpipeCloudWatchSession" \
...
#   export AWS_SESSION_TOKEN=$(echo $creds | awk '{print $3}')
  ...
  echo "Running CloudWatch metric fetch for account $account_id..."
  # Example CloudWatch command - replace with actual metrics needed
  aws cloudwatch list-metrics --region eu-west-2 > /tmp/cloudwatch_metrics_$account_id.json

  # Unset credentials after use
  unset AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY AWS_SESSION_TOKEN
done

echo "Cross-account CloudWatch access complete."